...
/* Canvas 3D globe — adapted from a TS / Tailwind / shadcn reference
   ...
   Recoloured to the brand-blue body + rose arcs/markers, and
   gated with IntersectionObserver + visibilitychange so the rAF
   loop pauses when the section is off-screen or the tab is hidden
   (perf contract). Drag-to-rotate kept; auto-rotates otherwise. */

// Decorative — no claim of "real" routes; just generic anchor points
// scattered around the sphere so the arcs draw nicely.
...
export default function BlueGlobe({
  dotColor = "rgba(140, 180, 255, ALPHA)",
  arcColor = "rgba(244, 100, 130, 0.6)",
  markerColor = "rgba(255, 200, 220, 1)",
  ...
  // pre-compute fibonacci-sphere dot positions once
  useEffect(() => {
    const dots = [];
    const numDots = 1200;
  ...
  // single-frame draw (no rAF here — the loop is owned by the effect
  // below so it can be paused / resumed on visibility / intersection)
  const drawFrame = useCallback(() => {
    ...
    // dots (only the visible hemisphere — z<0 in this projection)
    ...
    // connection arcs (rose) + a pulse riding each
    ...
    // markers (pulsing rose dots at endpoints)
  ...
  // owned rAF loop — only ticks while the canvas is in view AND the
  // tab is visible AND the user hasn't asked for reduced motion
  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;
