// Case study detail — PATCH to edit/publish/unpublish, DELETE to remove.
// Publishing is the admin's 1-click safety step (status pending → published).
...
const EDITABLE = ["title", "excerpt", "body", "cover_url", "status", "slug"];
...
export async function PATCH(request, { params }) {
  ...
  try {
    const res = await fetch(
      `${SUPABASE_URL}/rest/v1/case_studies?id=eq.${encodeURIComponent(id)}`,
      {
        method: "PATCH",
        headers: {
          apikey: SUPABASE_KEY,
